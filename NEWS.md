...
* Все пакеты -- изменение внутренней структуры всех нодов, нужно создавать новые скрипты;
- [NVP_Common] Добавлены ноды StringToNumber_Double, StringToNumber_Int64, ToString;

- [NVP_Common] Добавлена группа нодов Wait для создания искусственных задержек результатов частей скрипта;

- [NVP_Common] Квартирография -- добавлен второй сценарий расчета только для рассчитанных свойств;

- [NVP_nanoCAD_COM] Исправлены ноды из группы AcadHatch: AppendOuterLoop_Single и AppendOuterLoop_Group;

- [NVP_nanoCAD_COM] AcadBloc.GetAll_EntitiesMST - исправлено поведение выборки объектов NST;

- [NVP_ModelStudio_COM] Добавлен объект "Комната" - WrAECRoom к MST : Строительные решения;

- [NVP_ModelStudio_COM] Добавлен конструктор (Element_ConstructorCast2) для Element, для получения его от объекта модели AcadEntity;

**Прочие изменения**:

- Сформирована краткая справка по всем нодам [см. каталог](./docs/help);

- Скорректированы некоторые примеры в связи с изменениями в нодах всех библиотек;

- В тестовом виде добавлены пакеты для .NET API платформы nanoCAD и nanoCAD BIM Строительство, в сборки пока не вошли;
